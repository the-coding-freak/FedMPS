...
class REALWASTE(data.Dataset):
    ...
    def generate_ds(self, args, root):
        ...
        num_img = args.train_shots_max * args.num_users#10(222/20)*20=200

        data = []
        targets = torch.zeros([num_class * num_img])
        class_pathes = glob.glob(root + '/train'+"/*")  # 路径

        for i in range(num_class):
            class_path=class_pathes[i]
            imgs_paths_total=glob.glob(class_path + '/*')
            img_paths=random.sample(imgs_paths_total, num_img)
    ...
    def generate_ds_test(self, args, root):
        ...
        num_img = args.test_shots * args.num_users#4*20=80

        data = []
        # targets = torch.zeros([num_class * num_style * num_img])
        targets = torch.zeros([num_class * num_img])
        class_pathes = glob.glob(root + '/val' + "/*")  # path
        for i in range(num_class):
            class_path=class_pathes[i]
            imgs_paths_total = glob.glob(class_path + '/*')
            img_paths=random.sample(imgs_paths_total, num_img)
